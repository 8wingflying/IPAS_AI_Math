## 📘 常態分布 PDF

$$
f(x)=\frac{1}{\sqrt{2\pi\sigma^2}}\exp\left( -\frac{(x-\mu)^2}{2\sigma^2} \right)
$$

---

## 常態分布圖形
- PDF
- np.linspace(-4, 4, 400) 這段程式碼使用了 numpy 函式庫中的 linspace 函式:
  - np.linspace: 這個函式用於在指定區間內，生成一系列等間距的數字陣列
  - `-4`: 這是區間的起始值
  - `4`: 這是區間的結束值。
  - `400`: 這是 `num` 參數，指定要生成的樣本數量。在這個例子中，它會生成 400 個等間距的數字。
  - `np.linspace(-4, 4, 400)` 會生成一個包含 400 個數字的陣列，這些數字均勻分佈在 -4 到 4 之間（包含 -4 和 4）。
  - 這在繪製函式圖時經常用來創建 X 軸的數值。
```python
import matplotlib.pyplot as plt
import numpy as np

# Generate normal distribution plot
x = np.linspace(-4, 4, 400)
mu = 0
sigma = 1
y = 1/(np.sqrt(2*np.pi)*sigma) * np.exp(-(x-mu)**2/(2*sigma**2))

# 畫圖
plt.figure()
plt.plot(x, y)
plt.xlabel("x")
plt.ylabel("PDF")
plt.title("Standard Normal Distribution")
plt.grid(True)
```
- CDF
```python
import matplotlib.pyplot as plt
from scipy.stats import norm
import numpy as np

# Reuse the x-values from the previous plot
x = np.linspace(-4, 4, 400)

# Calculate the CDF using scipy.stats.norm.cdf
# For a standard normal distribution (mu=0, sigma=1)
cdf_values = norm.cdf(x, loc=0, scale=1)

# 畫圖
plt.figure()
plt.plot(x, cdf_values)
plt.xlabel("x")
plt.ylabel("CDF")
plt.title("Standard Normal Distribution CDF")
plt.grid(True)
plt.show()
```
## 🧪 SymPy 推導程式碼

```python
import sympy as sp

x = sp.Symbol('x', real=True)
mu = sp.Symbol('mu', real=True)
sigma = sp.Symbol('sigma', positive=True)

pdf = 1/(sp.sqrt(2*sp.pi)*sigma) * sp.exp(-(x-mu)**2 / (2*sigma**2))

EX = sp.integrate(x * pdf, (x, -sp.oo, sp.oo))
EX2 = sp.integrate(x**2 * pdf, (x, -sp.oo, sp.oo))

VarX = sp.simplify(EX2 - EX**2)

EX, VarX
```

---

## 📌 計算結果

- 期望值：$$ E[X] = \mu $$
- 變異數：$$ Var(X) = \sigma^2 $$

---
